...
const CELL_TYPE = [
  { id: '', name: '', upgradeCost: 0 },
  { id: '', name: '', upgradeCost: 40 },
  { id: '', name: '', upgradeCost: 30 },
  { id: '', name: '', upgradeCost: 20 },
]
...
export default function Game() {
  ...
  function moveTo(i, next) {
    ...
    if(players[i].position === next)
      return

    const newCells = cells.slice()
    newCells[players[i].position].players.delete(i)
  ...
  function trigerPay(i, j, cost) {
    setMessage(`player: ${players[i].name} pay {${cost}} to ${players[j].name}!`)
    const newPlayers = players.slice()

    if(newPlayers[i].cash >= cost) {
      newPlayers[i].cash -= cost
      newPlayers[j].cash += cost

      setPlayers(newPlayers)
      setTurn((turn + 1) % (newPlayers.length))
    } else {
      newPlayers[j].cash += newPlayers[i].cash
      newPlayers.splice(j, 1)

      if(newPlayers.length === 1) {
        setMessage(newPlayers[0] + 'wins!')
        setWinner(newPlayers[0])
      } else {
        setPlayers(newPlayers)
        setTurn((j + 1) % (newPlayers.length))
      }
    }
  }
  ...
  function handleMove(i, newDices) {
  ...
  function handleRoll(diceSize = DICE_NUMBER) {
    ...
    if(players[turn].status === STATUS_JAIL) {
      if(newDices.every(one => newDices[0] === one)) { // only the same results can change status(to move)
        updatePlayers(turn, 'status', STATUS_MOVE)
        setMessage('go out from jail.')
      } else {
        setTurn((turn + 1) % (players.length))
      }
    } else {
      handleMove(turn, newDices)
      handleStop(turn)
    }
  }
  ...
  return (
    <div>
      <div>
        <button disabled={ choice < 0 } onClick={ () => handleRoll() } onKeyUp={ handleRoll }>
          { players[Number(turn)].name } { choice < 0 ? "'s choice" : "'s turn" }
        </button>
...
function Cell({ name, type, players, ownedBy }) {
  ...
  // useEffect(() => {
  //    // use window.requestAnimationFrame() to make players move
  // })
